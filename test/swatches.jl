...
light_colors = sort(
    collect(pairs(Fathom.LIGHT_COLORS)); by = p -> oklab_l(p[2]),
    rev = true
)

lighter_colors = sort(
    collect(pairs(Fathom.LIGHTER_COLORS)); by = p -> oklab_l(p[2]),
    rev = true
)
darker_colors = sort(
    collect(pairs(Fathom.DARKER_COLORS)); by = p -> oklab_l(p[2]),
    rev = true
)
pelagic_colors = sort(
    collect(pairs(Fathom.PELAGIC_COLORS)); by = p -> oklab_l(p[2]),
    rev = true
)
...
for (row_idx, (group_name, colorset)) in enumerate(grouped_colors)
    ...
    for (col_idx, (name, c)) in enumerate(colorset)
        ...
        text!(
            ax, x0 + 0.5, y0 + 0.5, text = ltxt, color = txtcolor,
            align = (:center, :center), fontsize = 11
        )
        text!(
            ax, x0 + 0.5, y0 + 2.08, text = String(name),
